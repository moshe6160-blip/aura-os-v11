...
function renderBriefing(data) {
  if (data.error) { $("briefingBox").innerHTML = `<div class="error">${esc(data.error)}</div>`; return; }
  ...
  $("briefingBox").innerHTML = `
    ...
    ${calendar.length ? calendar.map(e => `
      <div class="item">
        <b>${esc(e.summary || "(No title)")}</b>
        <span>${fmtDate(e.start)}${e.location ? " · " + esc(e.location) : ""}</span>
      </div>`).join("") : `<div class="empty">No upcoming calendar items returned.</div>`}`;
}
